...
class AhoCorasickTest(unittest.TestCase):
    ...
    def testAllowOverlapsWithSharedPrefix(self):
	"""Exposes a bug in 0.9 reported by Michal Guerquin
	(http://michal.guerquin.com), where the continued search
	didn't work if any two keywords shared a prefix."""
	self.tree.add("I")
	self.tree.add("I went")
	self.tree.add("I went to")
	self.tree.make()
	results = list(self.tree.findall("I went to the store",
					 allow_overlaps=True))
    ...
    def testAllowOverlapsLongWithSharedPrefix(self):
	"""Just making my fix for testAllowOverlapsWithSharedPrefix
	doesn't mess up things when we look for longest matches."""
	self.tree.add("I")
	self.tree.add("I went")
	self.tree.add("I went to")
	self.tree.make()
	results = list(self.tree.findall_long("I went to the store",
                                              allow_overlaps=True))
	self.assertEqual([(0, 9)], results)
